...
export class AdaptiveHLSVideoConversionHandler {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const selectedVideo = await this.userInputService.askForVideoSelection(videoFiles);
      ...
      switch (outputSpace.toLowerCase()) {
        ...
        case "aws s3 bucket":
          ...
          console.log(chalk.blue(`Uploading to S3 bucket: ${s3Config.bucketName}, prefix: ${s3Prefix}`));
          await this.awsS3Service.uploadDirectory(tempDir, s3Config.bucketName, s3Prefix);

          fs.rmSync(tempDir, { recursive: true, force: true });
          console.log(chalk.gray(`Deleted temporary directory: ${tempDir}`));
          console.log(chalk.green(`✅ HLS conversion and upload to S3 bucket '${s3Config.bucketName}' complete!`));
          break;
